...
const { abiPolyfill, RegistryABI, RegistryExtras, GitHubHintABI, OperationsABI, BadgeRegABI, TokenRegABI, BadgeABI, TokenABI } = require('./abis.js');
...
function createBonds (options) {
	...
	class SubscriptionBond extends oo7.Bond { // eslint-disable-line no-unused-vars
		constructor (rpc) {
			super();
			this.rpc = rpc;
		}
		...
		initialise () {
			api()
				.subscribe(this.rpc, (_, n) => this.trigger(n))
	...
	class Signature extends oo7.ReactivePromise {
		constructor (message, from) {
			super([message, from], [], ([message, from]) => {
				api().parity
					.postSign(from, util.asciiToHex(message))
	...
	function call (addr, method, args, options) {
		...
		return api().eth
			.call(overlay({to: addr, data: data}, options))
			.then(decode);
	}
		...
		return new Proxy(f, {
			get (receiver, name) {
				if (typeof name === 'string' || typeof name === 'number') {
					...
					throw Error(`Weird value type to be subscripted by: ${typeof name}: ${JSON.stringify(name)}`);
				}
			}
		});
	};
	...
	bonds.findBlock = hashOrNumberBond => new TransformBond(
	...
	).subscriptable();// TODO: chain reorg that includes number x, if x is a number
	...
	bonds.uncle = (hashOrNumberBond, indexBond) => new TransformBond(
		...
		[hashOrNumberBond, indexBond], [/* onReorg */]
	).subscriptable();
	bonds.transaction = (hashOrNumberBond, indexOrNullBond) => new TransformBond(
		...
		[hashOrNumberBond, indexOrNullBond], [/* onReorg */]
	).subscriptable();
		...
		extras.forEach(i => {
			let f = function (...args) {
				...
				if (args.length !== expectedInputs) {
					throw new Error(`Invalid number of arguments to ${i.name}. Expected ${expectedInputs}, got ${args.length}.`);
				}
...
export const isAddressValid = h => h instanceof oo7.Bond ? h.map(Parity.Api.util.isAddressValid) : Parity.Api.util.isAddressValid(h);
export const toChecksumAddress = h => h instanceof oo7.Bond ? h.map(Parity.Api.util.toChecksumAddress) : Parity.Api.util.toChecksumAddress(h);
export const sha3 = h => h instanceof oo7.Bond ? h.map(Parity.Api.util.sha3) : Parity.Api.util.sha3(h);
...
export { RegistryABI, RegistryExtras, GitHubHintABI, OperationsABI, BadgeRegABI, TokenRegABI, BadgeABI, TokenABI };
...
export function splitValue (a) {
	var denom = 0;
	var base = new BigNumber('' + a);
	if ((base.gte(new BigNumber('10000000000000000')) && base.lt(new BigNumber('100000000000000000000000'))) || base.eq(0)) {
		denom = 6;
	} else {
		for (var aa = base; aa.gte(1000) && denom < denominations.length - 1; aa = aa.div(1000)) {
			denom++;
		}
	}
